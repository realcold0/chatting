...
public interface ChatRecordRepository extends JpaRepository<ChatRecord, Long> {
    ...
    @Query("select c from ChatRecord c where c.room_type = \'M\' and c.room_id = :groupId order by c.send_time")
    List<ChatRecord> findGroupChatById(@Param("groupId") Long groupId);

    @Query("select c from ChatRecord c where c.room_type = \'S\' and c.room_id = :singleId order by c.send_time")
